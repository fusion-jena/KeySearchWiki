...
Furthermore, a concrete use case of the dataset is demonstrated under [Experiments](#experiments) and the steps evaluating the correctness of relevant entities are presented under [Evaluation](#evaluation).
...
This is done by removing the target from the query if one of its keywords is a target descendant.
...
To evaluate the correctness of the relevant entities included in KeySearchWiki, the following steps should be followed: 
(Comparison against a selected baseline using Precision and Recall as metrics, refer to section 7 in the paper for more details)

1. run `npm run compareSPARQL` in the root folder. The output files can be found under `./eval/`.
2. run `npm run generatePlots` in the root folder to generate metrics plots. The output can be found under `./charts/`(all plots used in the paper)
